...
export async function verifyBiFoundation({ repositoryRoot = root, configPath, hostOs, hostArch } = {}) {
  ...
  if (config.schemaVersion !== 'chimpmaera.bi/foundation-config/v1' || config.platform !== 'linux/amd64' || config.enabledProfile !== 'bi001' || !Number.isInteger(config.hostPort) || config.hostPort < 1024 || config.hostPort > 65535) deny('BI_CONFIG_UNSUPPORTED_DENIED');
