...
export async function verifyBiSupersetM0() {
  ...
  if ((bootstrap.match(/SqlaTable\(/g) || []).length !== 3 || !bootstrap.includes('datasetCount":db.session.query(SqlaTable).count()')) deny('BI_SUPERSET_DATASET_IMPORT_DRIFT');
  ...
  for (const value of ['cm_discovery_s2_sales_profile','discovery-s2-projections.json','s2DiscoveryProjectionCount']) if (!bootstrap.includes(value)) deny('BI_DISCOVERY_S2_SUPERSET_PROJECTION_DRIFT');
