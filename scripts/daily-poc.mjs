...
function buildCoreArtifacts(manifest, manifestDigest, facts, previous, videoStatus) {
  ...
  const releaseNotes = `# ${releaseTitle}\n\nCandidate version: \`${manifest.targetVersion}\`\n\nSource: \`${manifest.source.base}\` → \`${manifest.source.head}\`\n\nThis is a prepared local candidate, not a published GitHub release.\n\n## User-visible highlights\n\n${markdownList(highlights, (item) => `**${item.id} — ${item.title}.** ${item.description} (cases: ${item.caseIds.join(", ")}; files: ${item.fileRefs.join(", ")})`)}\n\n## Evidence-bound claims\n\n${markdownList(claims, (claim) => `**${claim.id} [${claim.maturity}]** ${claim.statement} Evidence: ${claim.evidenceRefs.join(", ")}.`)}\n\n## Explicit non-claims\n\n${markdownList(claims.flatMap((claim) => claim.nonClaims.map((item) => ({ ...item, claimId: claim.id }))), (item) => `**${item.id}** (${item.claimId}) ${item.statement}`)}\n`;
  ...
  const readmePointer = `## Latest Daily POC candidate\n\n[${releaseTitle}](./release-notes.md) — \`${manifest.targetVersion}\` — local candidate only; publication disabled.\n`;
