...
test("single-node changes select the owner, downstream integrity and mandatory hard gates", () => {
  ...
  assert.deepEqual(result.selectedNodes, ["learning-routing-foundation", "openclaw-m1-4", "repository-integrity", "secure-default-proof", "vf-shadow-v2"]);
...
test("contract and cross-contract changes invalidate downstream dependants", () => {
  for (const changed of [
    ...
    assert.equal(result.mode, "IMPACTED_SHADOW");
    assert.deepEqual(result.selectedNodes, ["bi-execution-spine-v1", "integration-profile-v1", "learning-routing-foundation", "openclaw-m1-4", "repository-integrity", "secure-default-proof", "vf-contract-v1", "vf-shadow-v2"]);
  }
});
...
test("learning-routing changes select the complete foundation and downstream integrity gates", () => {
  ...
  assert.equal(result.mode, "IMPACTED_SHADOW");
  assert.deepEqual(result.selectedNodes, ["learning-routing-foundation", "openclaw-m1-4", "repository-integrity", "secure-default-proof"]);
...
test("integration profile changes select the bounded owner and downstream integrity gates", () => {
  ...
  assert.equal(result.mode, "IMPACTED_SHADOW");
  assert.deepEqual(result.selectedNodes, ["integration-profile-v1", "openclaw-m1-4", "repository-integrity", "secure-default-proof"]);
...
test("BI execution spine changes select the bounded owner and downstream integrity gates", () => {
  ...
  assert.equal(result.mode, "IMPACTED_SHADOW");
  assert.deepEqual(result.selectedNodes, ["bi-execution-spine-v1", "openclaw-m1-4", "repository-integrity", "secure-default-proof"]);
