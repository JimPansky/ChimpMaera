...
| External Superset_BI_Agent service boundary | **Released contract / optional external subsystem** | [External BI service contract](EXTERNAL-BI-SERVICE.md), focused CM adapter probes | CM stores only configurable URLs, expected release/contract versions and timeouts; BI source, Docker, volumes, secrets and database credentials remain outside CM |
