...
- Rebound the retained replay-conflict lifecycle probe to that same V2 route
  with a fresh otherwise-valid identity and deliberately invalid typed
  payload. The exact contract definition reaches the stable sanitized
  `TYPED_REQUEST_PAYLOAD_DENIED` boundary with zero effects.
