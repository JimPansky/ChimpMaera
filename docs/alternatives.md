---
title: When ChimpMaera is not the right tool
...
# When ChimpMaera is not the right tool

ChimpMaera is a local proof of concept for a specific problem: an Agent
...
| You need a hardened process or workload boundary | A sandbox or workload-isolation platform | ChimpMaera's Docker fixture is not a hostile-host security boundary. |
| Existing applications already execute correctly and you only need traces, metrics, or logs | An observability platform | Observation alone does not need ChimpMaera's proposal, approval, lease, or broker path. |
...
runtime. ChimpMaera's reference architecture explores the authority and
...
and verify its [capability evidence](./capabilities.md). ChimpMaera makes no
