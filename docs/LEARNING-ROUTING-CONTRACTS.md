...
## First-use advisory recommender and recorder

The additive first-use recommender is pure, deterministic, advisory-only, and conservative. It accepts only a digest-bound public context, explicit public evidence state, and a finite set of available CM model aliases. Its output always includes a model alias, thinking profile, workflow, confidence, reason codes, and an explicit static fallback. Private or malformed context, expanded authority, missing or unknown evidence, and unavailable aliases fail closed to that fallback. No recommendation executes work or grants routing authority.

The local NDJSON recorder is default-off: omitting `enabled: true` performs no filesystem access. When explicitly enabled, it appends only the minimized digest-bound recommendation and a caller-supplied timestamp; it never persists issue text, prompts, responses, code, paths, credentials, provider identifiers, or authority. This narrow recorder does not activate telemetry, retention policy, training ingestion, provider calls, or production routing. Rollback removes the additive module, tests, export, package entry, and this section; recorded canary files remain caller-owned local evidence.
